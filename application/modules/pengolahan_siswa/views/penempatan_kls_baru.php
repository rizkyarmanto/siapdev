    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input name="id_siswa_kelas" type="hidden">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="id_siswa" class="control-label">Siswa</label>
                            <select class="form-control myselect2" name="id_siswa" style="width: 100%;" >
                                <?php echo $siswa?>
                            </select>
                        </div>
                    </div>
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('pengolahan_siswa/showDataKelasSiswa')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#satkers').hide();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Siswa Kelas");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('pengolahan_siswa/siswaKelasAction')?>/"+"detail",
                dataType: "json",
                data: "id_siswa_kelas="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_siswa_kelas']").val(data.id_siswa_kelas);
                    $('#form-field').find("select[name='id_siswa']").html(data.siswa);
                    $('#form-field').find("select[name='id_kelas']").html(data.kelas);
                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Siswa Kelas");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var user  = $(this).attr('data-id_siswa_kelas');
            var nama      = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('pengolahan_siswa/siswaKelasAction')?>/"+action,
                dataType: "json",
                data: 'id_siswa_kelas='+user+'&status='+status,
                type: "POST",
              success: function(data) {
                    if (data.status == 1) {
                        var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('pengolahan_siswa/siswaKelasAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
