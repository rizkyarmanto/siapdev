...
class Data_master extends CI_Controller {
    ...
    public function show_jenjang_pendidikan() {
        ...
        $second_id[]    = array('id_sekolah', $this->session->userdata('id_sekolah'));
        ...
        foreach ($list as $l) {
            ...
            $row[] = '<input type="hidden" class="paramId " value="'.$l->id_jenjang.'"><input class="activeBtn" onChange="aktif(this)" type="checkbox" '.$status.'>';
    ...
    public function show_jenis_sosmed() {
        ...
        foreach ($list as $l) {
            ...
            $row[] = '<input type="hidden" class="paramId" value="'.$l->id_jenis_sosmed.'"><input class="activeBtn" onChange="aktif(this)" type="checkbox" '.$status.'>';
            // $row[] = '<input value="" type="checkbox" class="switch" data-on-text="On" data-off-text="Off" data-on-color="primary" data-off-color="default" checked="checked">';
            // $row[] = "test";
    ...
    function jenis_tarif() {
        ...
        $d['list_jenjang']      = $this->M_global->list_select8('m_jenjang_tbl', $this->session->userdata('id_sekolah'))->result_array();
        $d['list_jurusan']      = $this->M_global->list_select8('m_jurusan_tbl', $this->session->userdata('id_sekolah'))->result_array();
        $d['list_tingkat']      = $this->M_global->list_select8('m_tingkat_tbl', $this->session->userdata('id_sekolah'))->result_array();
        $d['list_gelombang']    = $this->M_global->list_select8('m_gelombang_tbl', $this->session->userdata('id_sekolah'))->result_array();
    ...
    public function show_jenis_tarif() {
        ...
        $list_jenjang   = $this->db->get_where('m_jenjang_tbl', ['id_sekolah' => $this->session->userdata('id_sekolah')])->result_array();
    ...
    function jenis_keringanan() {
        ...
        $d['list_jenjang'] = $this->db->get_where('m_jenjang_tbl', array('aktif' => 1, 'id_sekolah' => $this->session->userdata('id_sekolah')))->result_array();
        $d['list_jurusan'] = $this->db->get_where('m_jurusan_tbl', array('aktif' => 1, 'id_sekolah' => $this->session->userdata('id_sekolah')))->result_array();
        $d['list_tingkat'] = $this->db->get_where('m_tingkat_tbl', array('aktif' => 1, 'id_sekolah' => $this->session->userdata('id_sekolah')))->result_array();
        $d['list_tipe_transaksi'] = $this->M_global->list_select('m_tipe_transaksi_tbl')->result_array();
        $this->load->view('layout', $d);
    }
    ...
    public function jenis_keringanan_action($action) {
        ...
        if ($action == "add") {
            ...
            $data   = array(
                    ...
                    'tingkat'           => $tingkat,
                    'nominal'           => $nominal,
                    'id_nama_tarif'     => $id_tarif,
                    'id_sekolah'        => $this->session->userdata('id_sekolah')
                );
            $is_successs = $this->M_data->insert_master('m_keringanan_nilai_tbl', $data);
            $id_keringanan_nilai = $this->db->insert_id();
            if (!$is_successs) {
                $data_for_jsiswa_keringanan = array(
                    'id_sekolah' => $this->session->userdata('id_sekolah'),
                    'id_jurusan' => $id_jurusan,
                    'id_ta'      => $id_ta,
                    'id_jenjang' => $id_jenjang,
                    'tingkat'    => $tingkat,
                    'id_keringanan_nilai' => $id_keringanan_nilai                                                
                );
                $this->M_data->insert_siswa_keringanan('j_siswa_keringanan_tbl', $data_for_jsiswa_keringanan);
                $id_siswa_keringanan = $this->db->insert_id();

                $whr_tarnil = array('id_ta' => $id_ta, 'id_jenjang' => $id_jenjang, 'id_jurusan' => $id_jurusan, 'tingkat' => $tingkat, 'id_gelombang' => $id_gelombang, 'id_tarif' => $id_tarif, 'id_sekolah' => $this->session->userdata('id_sekolah'));
                $get_idTarnil = $this->M_transaksi->get_idTarnil($whr_tarnil)->row();
                $id_tarif_nilai = $get_idTarnil->id_tarif_nilai;

                $data_for_keringanan_item = array(
                                'id_siswa_keringanan' => $id_siswa_keringanan,
                                'id_tipe_transaksi'   => $id_tipe_transaksi,
                                'id_tarif_nilai'      => $id_tarif_nilai,
                                'id_keringanan_nilai' => $id_keringanan_nilai,
                                'id_sekolah'          => $this->session->userdata('id_sekolah')
                );
                $this->M_data->insert_keringanan_item('t_keringanan_item_tbl', $data_for_keringanan_item);
            ...
            $row        .= '<td style="padding:0px;height:39px;">'.$this->list_namaTarif().'</td>';
    ...
    public function show_jenis_keringanan() {
        ...
        // $list_jenjang   = $this->M_global->list_select('m_jenjang_tbl')->result_array();
        $list_jenjang   =   $this->db->get_where('m_jenjang_tbl', array('aktif' => 1, 'id_sekolah' => $this->session->userdata('id_sekolah')))->result_array();
        foreach ($list_jenjang as $lj) {
            array_push($column_order, $lj['jenjang']);
        }
            array_push($column_order, null);
        ...
        $second_id[]    = array('id_sekolah', $id_sekolah);
        ...
        foreach ($list as $l) {
            ...
            $row[] = $l->tipe_transaksi;
            foreach ($list_jenjang as $lj) {
                    $row[] = '<a class="text-warning" href="javascript:;" onClick="showNilai(this,'.$l->id_tipe_transaksi.')" data-keringanan="'.$l->keringanan.'" data-jenjang="'.$lj['jenjang'].'" data-id="'.$l->id_keringanan.'" data-filterId="'.$lj['id_jenjang'].'" data-filterColumn="id_jenjang"><i class="fa fa-search-plus"></i>&nbsp;<sup>('.$l->$lj['jenjang'].')</sup></a>';
            }
    ...
    public function show_gelombang_pend() {
        ...
        foreach ($list as $l) {
            ...
            $row[] = $no;
            $row[] = $l->gelombang;
            $row[] = $this->M_core->indonesian_date($l->tgl_mulai);
            $row[] = $this->M_core->indonesian_date($l->tgl_selesai);
                    if($l->aktif == '1') {
                        $status='checked="checked"';
                    } else {
                        $status='';
                    }
            $row[] = '<input type="hidden" class="paramId" value="'.$l->id_gelombang.'"><input class="activeBtn" onChange="aktif(this)" type="checkbox" '.$status.'>';
    ...
    public function show_siswa_status() {
        ...
        foreach ($list as $l) {
            ...
            $row[] = '<input type="hidden" class="paramId" value="'.$l->id_siswa_status.'"><input class="activeBtn" onChange="aktif(this)" type="checkbox" '.$status.'>';
            // $row[] = '<input value="" type="checkbox" class="switch" data-on-text="On" data-off-text="Off" data-on-color="primary" data-off-color="default" checked="checked">';
            // $row[] = "test";
    ...
    public function show_jurusan() {
        ...
        $second_id[]    =   ['id_sekolah', $this->session->userdata('id_sekolah')];
        $list           = $this->M_data->get_datatables($table, $column_order, $column_search, $order, $second_id);
        $data           = array();
        $no             = $_POST['start'];
        foreach ($list as $l) {
            ...
            $row[] = '<input type="hidden" class="paramId" value="'.$l->id_jurusan.'"><input class="activeBtn"  onChange="aktif(this)" type="checkbox" '.$status.'>';
...
// Jenis Tarif
    function siswa() {
        ...
        $d['gelombang'] = $this->M_core->listSelect($this->session->userdata('id_sekolah'), 'm_gelombang_tbl', 'gelombang', 'id_gelombang', '-- Pilih Gelombang--');
    ...
    function siswaAction($action){
        if($action == 'aktif'){

            $response = $this->actionActiveHandler('id_siswa', 'm_siswa_tbl');
            ...
            $nama_ibu       =   $this->input->post('nama$nama_ibu');
            ...
            if($dataFile['file_asli'][0] == '' || $dataFile['file_hash'][0] == ''){
                $data = $dataSiswaNoFile;
            }else{
                $data = $dataSiswaWithFile;
            }

            $insertUser =  $this->M_admin->insert_data('m_siswa_tbl', $data);
            ...
            $response = $this->actionInsertUpdateHandler($data['id_siswa'],'id_siswa', $data, 'm_siswa_tbl');
			...
			$response 		= $this->M_admin->get_master_spec('m_siswa_tbl', '*', $condition)->row_array();
            ...
            $response['gelombang']      =   $this->M_core->listSelected($this->session->userdata('id_sekolah'), 'm_gelombang_tbl', 'aktif', 1, 'id_gelombang', $response['id_gelombang'], 'id_gelombang', 'gelombang', '-- Pilih Gelombang --');
            $response['jurusan']        =   $this->M_core->listSelected($this->session->userdata('id_sekolah'), 'm_jurusan_tbl', 'aktif', 1, 'id_jurusan', $response['id_jurusan'], 'id_jurusan', 'jurusan', '-- Pilih Jurusan --');
            ...
            $response['siswaStatus']    =   $this->M_core->getListSelectedDefNol('m_siswa_status_tbl', 'aktif', 1, 'id_siswa_status', $response['id_status_siswa'], 'id_siswa_status', 'status', '-- Pilih Status Siswa --');
            $response['prov']           =   $this->M_core->getListSelectedDefNol('m_provinsi_tbl', 'aktif', 1, 'id_provinsi', $response['id_provinsi'], 'id_provinsi', 'nama_provinsi', '-- Pilih Provinsi --');
            
			$response['kab']            =   $this->M_core->getSelectedOne('m_kabupaten_tbl', 'aktif', 1, 'id_kabupaten', $response['id_kabupaten'], 'id_kabupaten', 'nama_kabupaten', '-- Pilih Kabupaten --');
			$response['kec']            =   $this->M_core->getSelectedOne('m_kecamatan_tbl', 'aktif', 1, 'id_kecamatan', $response['id_kecamatan'], 'id_kecamatan', 'nama_kecamatan', '-- Pilih Kecamatan --');
            $response['kel']            =   $this->M_core->getSelectedOne('m_kelurahan_tbl', 'aktif', 1, 'id_kelurahan', $response['id_kelurahan'], 'id_kelurahan', 'nama_kelurahan', '-- Pilih Kelurahan --');
            
            $response['fotoSiswa']  =   '<img src="'.$response['foto_locate'].'/'.$response['foto_hash'].'" width="50%">';
    ...
    public function show_siswa() {
		...
		if($id_gelombang != 0) {
			$condition[]  = ['id_gelombang', $id_gelombang, 'where'];
		}
		if($id_jurusan != 0) {
			$condition[]  = ['id_jurusan', $id_jurusan, 'where'];
		}
		
        $column_order   = array(null, 'nama', 'nis', 'teks_ta', 'jenjang', 'jurusan', 'nm_kelas', null, null); 
        $column_search  = array('nama', 'nis', 'teks_ta', 'jenjang', 'jurusan', 'nm_kelas'); 
        ...
        foreach ($list_data as $ld) {
            ...
            if ($status == 1) {
				$html_stat 	= '<input class="action activeBtn" data-action="aktif" data-id_siswa="'.$ld->id_siswa.'" 
								type="checkbox" id="lbl-'.$ld->nama.'" checked switch="none" >
                                    <label for="lbl-'.$ld->id_siswa.'" data-on-label="On" data-off-label="Off"></label>';
        	} else {
				$html_stat 	= '<input class="action activeBtn" data-action="aktif" data-id_siswa="'.$ld->id_siswa.'" 
            ...
            $row[] = $html_stat;
            $row[] = '&nbsp;<button 
                                type="button" data-id="'.$ld->id_siswa.'" data-action="edit" 
                        ...
                        &nbsp;<button 
                                type="button" data-id="'.$ld->id_siswa.'" data-master="m_siswa_tbl" data-col="id_siswa" data-action="delete" 
    ...
    private function list_jurusan() {   
        $list           = $this->M_global->list_select('m_jurusan_tbl')->result_array();
            ...
            $header    .= '<option value="99">Semua Jurusan</option>';
            $data       = '';
            foreach ($list as $l) {
    ...
    private function list_tingkat() {   
        $list           = $this->M_global->list_select3('m_tingkat_tbl')->result_array();
            ...
            $header    .= '<option value="99">Semua Tingkat</option>';
            $data       = '';
            foreach ($list as $l) {
    ...
    private function list_gelombang() {   
            $list       = $this->M_global->list_select5('m_gelombang_tbl', 'id_sekolah', $this->session->userdata('id_sekolah'))->result_array();
    ...
    public function showSekolah(){
        ...
        foreach ($list_data as $ld) {
            ...
            if ($status == 1) {
				$html_stat 	= '<input class="action activeBtn" data-action="aktif" data-id_sekolah="'.$ld->id_sekolah.'" 
    ...
    public function showMapel(){
        ...
        foreach ($list_data as $ld) {
            ...
            if ($status == 1) {
				$html_stat 	= '<input class="action activeBtn" data-action="aktif" data-id_mapel="'.$ld->id_mapel.'" 
    ...
    function guru(){
        ...
        $d['gender']    = $this->M_core->getListSelectDefNol('m_gender', 'aktif', 1, 'id_gender', 'gender', '-- Pilih Gender --');
        $d['sekolah']   = $this->M_core->listSelect($this->session->userdata('id_sekolah'), 'm_sekolah', 'id_sekolah', 'nm_sekolah', '-- Pilih Sekolah --');
        $d['provinsi']   = $this->M_core->getListSelectDefNol('m_provinsi_tbl', 'aktif', 1, 'id_provinsi', 'nama_provinsi', '-- Pilih Provinsi --');
        $this->load->view('layout', $d);
    }
    ...
    public function showGuru(){
        ...
        foreach ($list_data as $ld) {
            ...
            if ($status == 1) {
				$html_stat 	= '<input class="action activeBtn" data-action="aktif" data-id_guru="'.$ld->id_guru.'" 
    ...
    function guruAction($action){
        if($action == 'aktif'){
            ...
            $response['sl_sekolah'] =   $this->M_core->listSelected($this->session->userdata('id_sekolah'), 'm_sekolah', 'aktif', 1, 'id_sekolah', $response['id_sekolah'], 'id_sekolah', 'nm_sekolah', '-- Pilih Gender --');
            
            $response['sl_prov']    =   $this->M_core->getListSelectedDefNol('m_provinsi_tbl', 'aktif', 1, 'id_provinsi', $response['id_provinsi'], 'id_provinsi', 'nama_provinsi', '-- Pilih Provinsi --');
			$response['sl_kab']     =   $this->M_core->getSelectedOne('m_kabupaten_tbl', 'aktif', 1, 'id_kabupaten', $response['id_kabupaten'], 'id_kabupaten', 'nama_kabupaten', '-- Pilih Kabupaten --');
			$response['sl_kec']     =   $this->M_core->getSelectedOne('m_kecamatan_tbl', 'aktif', 1, 'id_kecamatan', $response['id_kecamatan'], 'id_kecamatan', 'nama_kecamatan', '-- Pilih Kecamatan --');
            $response['sl_kel']     =   $this->M_core->getSelectedOne('m_kelurahan_tbl', 'aktif', 1, 'id_kelurahan', $response['id_kelurahan'], 'id_kelurahan', 'nama_kelurahan', '-- Pilih Kelurahan --');
            $response['fotoGuru']  =   '<img src="'.$response['foto_locate'].'/'.$response['foto_hash'].'" width="50%">';
        }
    ...
    public function showTingkat(){
        ...
        foreach ($list_data as $ld) {
            ...
            if ($status == 1) {
				$html_stat 	= '<input class="action activeBtn" data-action="aktif" data-id_tingkat="'.$ld->id_tingkat.'" 
    ...
    public function show_siswa_psb() {
        $id_ta          = $this->input->post('id_ta');
        $id_jenjang     = $this->input->post('id_jenjang');
        $id_jurusan     = $this->input->post('id_jurusan');
        $tahun          = date('Y');
        ...
        $table          = 'v_psb';
        $column_order   = array(null, 'no_formulir', 'nama', 'asal_sekolah', 'nama_ortu', 'jenjang', 'jurusan', 'status', null); //set column field database for datatable orderable
        $column_search  = array('nama', 'no_formulir', 'teks_ta', 'jenjang', 'jurusan', 'asal_sekolah', 'nama_ortu', 'status'); //set column field database for datatable searchable 
        $order          = array('nama' => 'asc'); // default order 

        $second_id      = array();
        if ($tahun) {
            $second_id[]    = array('tahun', $tahun);
        }
        if ($id_sekolah) {
            $second_id[]    = array('id_sekolah', $id_sekolah);
        }
        ...
        foreach ($list as $l) {
            ...
            // $row[] = '';
            $row[]  = '<a class="getNoform text-primary" data-id="'.$l->no_formulir.'" href="javascript:;" data-toggle="tooltip" title="Pilih Siswa" data-original-title="Detaail"><i class="fa fa-check-square-o"></i></a>';
    ...
    private function list_namaTarif() {   
        $list       = $this->M_global->list_select5('m_tarif_tbl', 'id_sekolah', $this->session->userdata('id_sekolah'))->result_array();
        $header     = '<select style="height:100%;width:100%;" name="id_tarif">';
        $data       = '';
        // var_dump($list);
        foreach ($list as $l) {
            $data   .= '<option value="'.$l['id_tarif'].'">'.$l['nama_tarif'].'</option>';
        }
        $footer     = '</select>';
