    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input name="id_mapel" type="hidden">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="nm_mapel" class="control-label">Nama Matapelajaran</label>
                            <input name="nm_mapel" type="text" class="form-control field" id="nm_mapel">
                        </div>
                    </div>
                </div>  
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input name="keterangan" type="text" class="form-control field" id="keterangan">
                        </div>
                    </div>
                </div>  
            ...
            <div class="modal-footer">
                ...
                <button type="button" data-action="simpan" id="simpan" class="action btn btn-info waves-effect waves-light">Simpan</button>
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('data_master/showMapel')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Mapel");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('data_master/mapelAction')?>/"+"detail",
                dataType: "json",
                data: "id_mapel="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_mapel']").val(data.id_mapel);
                    $('#form-field').find("input[name='nm_mapel']").val(data.nm_mapel);
                    $('#form-field').find("input[name='keterangan']").val(data.keterangan);

                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Mapel");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var user  = $(this).attr('data-id_mapel');
            var nama  = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('data_master/mapelAction')?>/"+action,
                dataType: "json",
                data: 'id_mapel='+user+'&status='+status,
                type: "POST",
              success: function(data) {
                    if (data.status == 1) {
                        var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('data_master/mapelAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
