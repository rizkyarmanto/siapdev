    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header btn-success">
                ...
                <h4 class="modal-title">Strategi</h4>
            </div>
            <div class="modal-body">
                <input name="id_strategi" type="hidden">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="kd_strategi" class="control-label">Kode Strategi</label>
                            <input name="kd_strategi" type="text" class="form-control" id="kd_strategi">
                        </div>
                    </div>
                </div>  
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="nm_strategi" class="control-label">Nama Strategi</label>
                            <input name="nm_strategi" type="text" class="form-control" id="nm_strategi">
                        </div>
                    </div>
                </div>  
                <div class="row">
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('data_master/showStrategi')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Strategi");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('data_master/strategiAction')?>/"+"detail",
                dataType: "json",
                data: "id_strategi="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_strategi']").val(data.id_strategi);
                    $('#form-field').find("input[name='nm_strategi']").val(data.nm_strategi);
                    $('#form-field').find("input[name='kd_strategi']").val(data.kd_strategi);
                    $('#form-field').find("input[name='keterangan']").val(data.keterangan);

                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Strategi");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var user  = $(this).attr('data-id_strategi');
            var nama  = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('data_master/strategiAction')?>/"+action,
                dataType: "json",
                data: 'id_strategi='+user+'&status='+status,
                type: "POST",
              success: function(data) {
                    if (data.status == 1) {
                        var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('data_master/strategiAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
