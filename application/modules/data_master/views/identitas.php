    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input name="id_identitas" type="hidden">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="nm_identitas" class="control-label">Nama Identitas</label>
                            <input name="nm_identitas" type="text" class="form-control" id="nm_identitas">
                        </div>
                    </div>
                </div>  
                <div class="row">
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('data_master/showIdentitas')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Identitas");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('data_master/identitasAction')?>/"+"detail",
                dataType: "json",
                data: "id_identitas="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_identitas']").val(data.id_identitas);
                    $('#form-field').find("input[name='nm_identitas']").val(data.nm_identitas);
                    $('#form-field').find("input[name='keterangan']").val(data.keterangan);

                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Identitas");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var user  = $(this).attr('data-id_identitas');
            var nama  = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('data_master/identitasAction')?>/"+action,
                dataType: "json",
                data: 'id_identitas='+user+'&status='+status,
                type: "POST",
              success: function(data) {
                    if (data.status == 1) {
                        var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('data_master/identitasAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
