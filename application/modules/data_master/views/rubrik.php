    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header btn-success">
                ...
                <h4 class="modal-title">Rubrik</h4>
            </div>
            <div class="modal-body">
                <input name="id_rubrik" type="hidden">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="id_strategi" class="control-label">Strategi</label>
                            <select class="form-control myselect" name="id_strategi" style="width: 100%;" id="id_strategi">
                                <?php echo $strategi;?>
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="nm_rubrik" class="control-label">Nama Rubrik</label>
                            <input name="nm_rubrik" type="text" class="form-control field" id="nm_rubrik">
                        </div>
                    </div>
                </div>  
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input name="kriteria" type="text" class="form-control field" id="kriteria">
                        </div>
                    </div>
                </div>  
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="banyak_nilai" class="control-label">Banyak Nilai</label>
                            <input name="banyak_nilai" type="text" class="form-control field" id="banyak_nilai">
                        </div>
                    </div>
                </div> 
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('data_master/showRubrik')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Rubrik");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('data_master/rubrikAction')?>/"+"detail",
                dataType: "json",
                data: "id_rubrik="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_rubrik']").val(data.id_rubrik);
                    $('#form-field').find("input[name='nm_rubrik']").val(data.nm_rubrik);
                    $('#form-field').find("input[name='kriteria']").val(data.kriteria);
                    $('#form-field').find("input[name='kriteria']").val(data.kriteria);
                    $('#form-field').find("input[name='bobot']").val(data.bobot);
                    $('#form-field').find("input[name='banyak_nilai']").val(data.banyak_nilai);
                    $('#form-field').find("select[name='id_strategi']").html(data.sl_strategi);

                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Rubrik");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var user  = $(this).attr('data-id_rubrik');
            var nama  = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('data_master/rubrikAction')?>/"+action,
                dataType: "json",
                data: 'id_rubrik='+user+'&status='+status,
                type: "POST",
              success: function(data) {
                    if (data.status == 1) {
                        var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('data_master/rubrikAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
