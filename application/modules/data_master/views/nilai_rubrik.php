    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header btn-success">
                ...
                <h4 class="modal-title">Nilai Rubrik</h4>
            </div>
            <div class="modal-body">
                <input name="id_nilai_rubrik" type="hidden">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="id_rubrik" class="control-label">Rubrik</label>
                            <select class="form-control myselect field" name="id_rubrik" style="width: 100%;" id="id_rubrik">
                                <?php echo $rubrik;?>
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input name="nilai" type="text" class="form-control field" id="nilai">
                        </div>
                    </div>
                </div>  
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="indikasi" class="control-label">Indikasi</label>
                            <input name="indikasi" type="text" class="form-control field" id="indikasi">
                        </div>
                    </div>
                </div>  
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('data_master/showNilaiRubrik')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Nilai Rubrik");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('data_master/nilaiRubrikAction')?>/"+"detail",
                dataType: "json",
                data: "id_rubrik="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_nilai_rubrik']").val(data.id_nilai_rubrik);
                    $('#form-field').find("input[name='nilai']").val(data.nilai);
                    $('#form-field').find("input[name='indikasi']").val(data.indikasi);
                    $('#form-field').find("select[name='id_rubrik']").html(data.sl_rubrik);

                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Nilai Rubrik");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var user  = $(this).attr('data-id_nilai_rubrik');
            var nama  = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('data_master/nilaiRubrikAction')?>/"+action,
                dataType: "json",
                data: 'id_nilai_rubrik='+user+'&status='+status,
                type: "POST",
              success: function(data) {
                    if (data.status == 1) {
                        var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('data_master/nilaiRubrikAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
