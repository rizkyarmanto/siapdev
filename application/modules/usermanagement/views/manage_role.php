                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="nm_role" class="control-label">Nama Role</label>
                            <input name="nm_role" type="text" class="form-control" id="nm_role">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="kd_role" class="control-label">Kode Role</label>
                            <input name="kd_role" type="text" class="form-control" id="kd_role">
                        </div>
                    </div>
                </div>            
                <div class="row">
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('usermanagement/showRole')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#satkers').hide();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Role");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('usermanagement/roleAction')?>/"+"detail",
                dataType: "json",
                data: "id_role="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_role']").val(data.id_role);
                    $('#form-field').find("input[name='nm_role']").val(data.nm_role);
                    $('#form-field').find("input[name='kd_role']").val(data.kd_role);
                    $('#form-field').find("input[name='keterangan']").val(data.keterangan);
                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Role");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var role  = $(this).attr('data-id_role');
            var nama      = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
              url : "<?php echo base_url('usermanagement/roleAction')?>/"+action,
              dataType: "json",
              data: 'id_role='+role+'&status='+status,
              type: "POST",
              success: function(data) {
                if (data.status == 1) {
                  var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('usermanagement/roleAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
