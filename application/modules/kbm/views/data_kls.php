            ...
            <div class="pull-right">
              <!-- <button type="button" class="btn btn-primary btn-sm action" data-action="add"  data-toggle="tooltip" title="Tambah" data-original-title="Tambah">
                <i class="glyphicon glyphicon-plus"></i></button> -->
                 ...
                 <a href="<?php echo base_url('kbm/input_data_kls')?>" class="btn btn-primary btn-sm action">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select class="form-control myselect2" name="id_ta" style="width: 100%;" >
                                <?php echo $ta?>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="id_tingkat" class="control-label">Tingkat</label>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select class="form-control myselect2" name="id_jenjang" style="width: 100%;" >
                                <?php echo $jenjang?>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="urutan_kelas" class="control-label">Urutan Kelas</label>
                            <input name="urutan_kelas" type="text" class="form-control" id="urutan_kelas">
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="nm_kelas" class="control-label">Nama Kelas</label>
                            <input name="nm_kelas" type="text" class="form-control" id="nm_kelas" readonly>
                        </div>
                    </div>
                </div>  
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="gen" class=" btn btn-warning waves-effect waves-light">Generate Kelas</button>
            </div>
        </div>
    </div>
  </div>
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('kbm/showDataKls')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    $('#gen').click(function(){
        $('#form-field').find("input[name='nm_kelas']").val(  
            $('#form-field').find("select[name='id_tingkat'] option:selected").text() +' '+ 
            $('#form-field').find("select[name='id_jurusan'] option:selected").text() +' '+ 
            $('#form-field').find("input[name='urutan_kelas']").val()
        );
    });
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Kelas");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('kbm/kelasAction')?>/"+"detail",
                dataType: "json",
                data: "id_kelas="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_kelas']").val(data.id_kelas);
                    $('#form-field').find("input[name='urutan_kelas']").val(data.urutan_kelas);
                    // $('#form-field').find("input[name='tingkat']").val(data.tingkat);
                    $('#form-field').find("select[name='id_tingkat']").html(data.tingkat);
            ...
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Kelas");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var id  = $(this).attr('data-id_kelas');
            var nama  = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('kbm/kelasAction')?>/"+action,
                dataType: "json",
          ...
          $.ajax({
            url : "<?php echo base_url('kbm/kelasAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
