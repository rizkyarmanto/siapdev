            ...
            <div class="pull-right">
              <!-- <button type="button" class="btn btn-primary btn-sm action" data-action="add"  data-toggle="tooltip" title="Tambah" data-original-title="Tambah">
                <i class="glyphicon glyphicon-plus"></i></button> -->
                 ...
                 <a href="<?php echo base_url('kbm/input_walikelas')?>" class="btn btn-primary btn-sm action">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header btn-success">
                ...
                <h4 class="modal-title">Walikelas</h4>
            </div>
            <div class="modal-body">
                <input name="id_walikelas" type="hidden">
                <div class="row">
                    <div class="col-md-12">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select class="form-control myselect2" name="id_guru" style="width: 100%;" >
                                <?php echo $guru?>
                            </select>
                        </div>
                    </div>
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('kbm/showDataWalikelas')?>";
  var   totalClmn;
  var   table1;
  $(document).ready(function() {
    ...
    function show_modal(data) {
        ...
        if (mode == "add") {
            $('#form-field')[0].reset();
            $('#form-field').children('.modal').find('.modal-title').text("Tambah Walikelas");
            $('#form-field').children('.modal').modal('show'); 
        } 
        else if (mode == "edit") {
            $.ajax({
                url : "<?php echo base_url('kbm/walikelasAction')?>/"+"detail",
                dataType: "json",
                data: "id_walikelas="+data,
                type: "POST",
                success: function(data) {
                    $('#form-field').find("input[name='id_walikelas']").val(data.id_walikelas);
                    $('#form-field').find("select[name='id_kelas']").html(data.kelas);
                    $('#form-field').find("select[name='id_guru']").html(data.guru);
                }
            })
            
            $('#form-field').children('.modal').find('.modal-title').text("Ubah Walikelas");
            $('#form-field').children('.modal').modal('show');
    ...
    $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            var id  = $(this).attr('data-id_walikelas');
            var nama  = $(this).closest("tr").find("td:eq(2)").text();

            $.ajax({
                url : "<?php echo base_url('kbm/walikelasAction')?>/"+action,
                dataType: "json",
                data: 'id_walikelas='+id+'&status='+status,
                type: "POST",
              success: function(data) {
                    if (data.status == 1) {
                        var text    = 'aktifkan';
          ...
          $.ajax({
            url : "<?php echo base_url('kbm/walikelasAction')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
